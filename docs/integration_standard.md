# xnat_volview_plugin: Proxy Site Integration Standard
...
- Publish the remote entry under an authenticated path served by this plugin so the proxy can load it at runtime (default expectation: `https://<xnat-host>/plugins/xnat_volview_plugin/remoteEntry.js`).
...
2. Deploy to a development XNAT stack and verify `curl -I https://<xnat-host>/plugins/xnat_volview_plugin/remoteEntry.js` returns `200`.
...
## Rationale
Module Federation keeps the proxy site's Apps menu dynamic while still letting plugins ship independently. Following this standard ensures every xnat_volview_plugin release remains compatible with the automatic plugin discovery pipeline.
